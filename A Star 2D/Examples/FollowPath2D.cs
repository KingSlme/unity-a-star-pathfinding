...
{   
    ...
    {   
        ...
        if (_path.Length > 0)
        {   
            // Ignore Y direction for 2D movement
            Vector3 newPosition = Vector3.MoveTowards(transform.position, new Vector3(_path[0].x, transform.position.y, _path[0].z), _speed * Time.deltaTime);
            transform.position = newPosition;
            if (transform.position == new Vector3(_path[0].x, transform.position.y, _path[0].z))
            {
